# ma_cross_volume_filter_backtest.py
# Sprint #5 Test A: Golden Cross + Volume Filter
...
class GoldenCrossVolumeFilterStrategy(bt.Strategy):
    """
    Golden Cross Strategy with Volume Filter Enhancement
    
    Entry Rules:
    1. 50-day MA crosses above 200-day MA (Golden Cross)
    2. Volume on crossover day > 150% of 20-day average volume
    ...
    params = (
        ...
        ('volume_period', 20),    # 20-day volume MA
        ('volume_threshold', 1.5), # 150% volume threshold
        ('cost_model', None)
    )
    ...
    def __init__(self):
        ...
        self.volume_filter_rejections = 0
        
    def next(self):
        """
        Execute Golden Cross + Volume Filter logic
        """
        
        # Skip if we don't have enough data for indicators
        if len(self.data) < max(self.p.slow_period, self.p.volume_period):
            return
        
        # Buy signal: Fast MA crosses above Slow MA + Volume Filter
        if self.crossover > 0 and not self.position:
            ...
            if volume_ratio >= self.p.volume_threshold:
                ...
                self.volume_filter_rejections += 1
    ...
    def get_performance_metrics(self):
        ...
        if self.trade_count == 0:
            return {
                ...
                'volume_rejections': self.volume_filter_rejections
            }
...
def run_volume_filter_backtest(ticker, data_dir, initial_cash=10000):
    """
    Run Golden Cross + Volume Filter backtest for a single stock
    """
    
    print(f"\nRunning Golden Cross + Volume Filter backtest for {ticker}...")
    ...
    # Add strategy
    cerebro.addstrategy(GoldenCrossVolumeFilterStrategy, cost_model=cost_model)
    ...
    results_data = {
        ...
        'strategy': 'Golden Cross + Volume Filter',
...
def run_sprint_5_volume_filter_test():
    """
    Run Golden Cross + Volume Filter test across entire universe
    """
    ...
    print("=" * 80)
    print("SPRINT #5 TEST A: GOLDEN CROSS + VOLUME FILTER")
    print("=" * 80)
    print("Strategy: Golden Cross + Volume Confirmation Filter")
    print("Filter Rule: Volume > 150% of 20-day average on crossover day")
    print("Hypothesis: High volume confirms genuine trend changes")
    print("=" * 80)
    ...
    for ticker in UNIVERSE:
        result = run_volume_filter_backtest(ticker, DATA_DIR)
        if result:
    ...
    results_file = os.path.join(RESULTS_DIR, f'volume_filter_results_{timestamp}.json')
    
    volume_filter_results = {
        'test': 'Sprint #5 Test A - Volume Filter',
        'test_date': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
        'strategy': 'Golden Cross + Volume Filter (150% of 20-day avg)',
        'universe': UNIVERSE,
    ...
    with open(results_file, 'w') as f:
        json.dump(volume_filter_results, f, indent=2, default=str)
    
    print(f"\n" + "=" * 60)
    print("VOLUME FILTER TEST COMPLETE")
    print("=" * 60)
    print_aggregate_results(aggregate_metrics, "Volume Filter")
    print(f"Results saved to: {results_file}")
    
    return results_file, volume_filter_results
...
if __name__ == '__main__':
    results_file, results = run_sprint_5_volume_filter_test()
    print(f"\nSprint #5 Volume Filter test complete.")