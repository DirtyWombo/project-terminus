# sprint_6_bollinger_mean_reversion.py
# Sprint #6 Test B: Bollinger Band Mean Reversion Strategy
...
class BollingerMeanReversionStrategy(bt.Strategy):
    ...
    Entry Rules:
    1. Price closes below lower Bollinger Band (oversold)
    2. Maximum 3 simultaneous positions
    
    Exit Rules:
    1. Price closes above upper Bollinger Band (overbought), OR
    2. Stop-loss at -8%
    
    Hypothesis: Price movements outside Bollinger Bands represent statistical extremes
    """
    params = (
        ('bb_period', 20),        # 20-period Bollinger Bands
        ('bb_deviation', 2.0),    # 2 standard deviations
    ...
    def __init__(self):
        ...
        # Signal tracking
        self.lower_band_signals = 0
        self.trades_from_lower_band = 0
        self.stop_loss_exits = 0
        self.upper_band_exits = 0
    ...
    def next(self):
        """
        Execute Bollinger Band Mean Reversion logic
        """
        
        # Skip if insufficient data for Bollinger Bands
        ...
        # Entry Logic: Price below lower band and position limits
        if (current_price < lower_band and 
            not self.position and 
            self.position_count < self.p.max_positions):
            
            self.lower_band_signals += 1
            
            # Buy on lower band breach (oversold)
            order = self.buy()
            if order:
                self.position_entry_prices[self.data._name] = current_price
                self.position_count += 1
                self.trades_from_lower_band += 1
        
        # Exit Logic: Price above upper band OR stop-loss
        elif self.position:
            ...
            # Exit on upper band breach (overbought)
            if current_price > upper_band:
                self.close()
                self.upper_band_exits += 1
    ...
    def get_performance_metrics(self):
        """
        Calculate comprehensive performance metrics for Bollinger strategy
        """
        ...
        if self.trade_count == 0:
            return {
                ...
                'expectancy': 0,
                'profit_factor': 0,
                'lower_band_signals': self.lower_band_signals,
                'trades_from_lower_band': self.trades_from_lower_band,
                'signal_conversion_rate_pct': 0,
                'stop_loss_exits': self.stop_loss_exits,
                'upper_band_exits': self.upper_band_exits
            }
        ...
        signal_conversion_rate = (self.trades_from_lower_band / max(1, self.lower_band_signals)) * 100
...
def run_bollinger_mean_reversion_backtest(ticker, data_dir, initial_cash=10000):
    """
    Run Bollinger Band Mean Reversion backtest for a single stock
    """
    
    print(f"\nRunning Bollinger Mean Reversion backtest for {ticker}...")
    ...
    # Add strategy
    cerebro.addstrategy(BollingerMeanReversionStrategy, cost_model=cost_model)
    ...
    results_data = {
        ...
        # Bollinger-specific analysis
        'bollinger_analysis': {
            'strategy_type': 'Bollinger Band Mean Reversion (Lower band entry, Upper band exit)',
            'lower_band_signals': strategy_metrics['lower_band_signals'],
            'trades_executed': strategy_metrics['trades_from_lower_band'],
            'signal_conversion_rate_pct': strategy_metrics['signal_conversion_rate_pct'],
            'stop_loss_exits': strategy_metrics['stop_loss_exits'],
            'upper_band_exits': strategy_metrics['upper_band_exits']
        }
    }
    
    # Print summary
    sharpe_ratio = results_data['performance']['sharpe_ratio']
    sharpe_display = f"{sharpe_ratio:.2f}" if sharpe_ratio is not None and not np.isnan(sharpe_ratio) else "N/A"
    ...
    print(f"  Lower Band Signals: {strategy_metrics['lower_band_signals']}")
...
def run_sprint_6_bollinger_test():
    """
    Run Bollinger Band Mean Reversion test across entire universe
    """
    ...
    print("=" * 80)
    print("SPRINT #6 TEST B: BOLLINGER BAND MEAN REVERSION STRATEGY")
    print("=" * 80)
    print("Strategy: Bollinger Band Mean Reversion (Lower band entry, Upper band exit)")
    print("Risk Management: 8% stop-loss, max 3 positions")
    print("Hypothesis: Price outside bands represents statistical extremes")
    print("=" * 80)
    ...
    for ticker in UNIVERSE:
        result = run_bollinger_mean_reversion_backtest(ticker, DATA_DIR)
        if result:
    ...
    results_file = os.path.join(RESULTS_DIR, f'bollinger_mean_reversion_results_{timestamp}.json')
    
    bollinger_results = {
        'test': 'Sprint #6 Test B - Bollinger Band Mean Reversion',
        'test_date': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
        'strategy': 'Bollinger Band Mean Reversion (Lower band entry, Upper band exit, 8% stop)',
        'universe': UNIVERSE,
    ...
    with open(results_file, 'w') as f:
        json.dump(bollinger_results, f, indent=2, default=str)
    
    print(f"\n" + "=" * 60)
    print("BOLLINGER BAND MEAN REVERSION TEST COMPLETE")
    print("=" * 60)
    print_aggregate_results(aggregate_metrics, "Bollinger Band Mean Reversion")
    print(f"Results saved to: {results_file}")
    
    return results_file, bollinger_results
...
def calculate_aggregate_metrics(all_results):
    ...
    total_lower_band_signals = 0
    total_trades_from_lower_band = 0
    total_stop_loss_exits = 0
    total_upper_band_exits = 0
    
    for ticker, result in all_results.items():
        perf = result['performance']
        trading = result['trading']
        
        if (perf['sharpe_ratio'] is not None and 
            not np.isnan(perf['sharpe_ratio']) and 
            perf['sharpe_ratio'] != 0):
        ...
        total_lower_band_signals += trading.get('lower_band_signals', 0)
        total_trades_from_lower_band += trading.get('trades_from_lower_band', 0)
        total_stop_loss_exits += trading.get('stop_loss_exits', 0)
        total_upper_band_exits += trading.get('upper_band_exits', 0)
    ...
    aggregate = {
        ...
        'total_lower_band_signals': total_lower_band_signals,
        'total_trades_from_lower_band': total_trades_from_lower_band,
        'aggregate_signal_conversion_pct': (total_trades_from_lower_band / max(1, total_lower_band_signals)) * 100,
        'total_stop_loss_exits': total_stop_loss_exits,
        'total_upper_band_exits': total_upper_band_exits,
...
def print_aggregate_results(aggregate_metrics, strategy_name):
    """
    Print aggregate results for the Bollinger strategy
    """
    ...
    print(f"\nBOLLINGER BAND SIGNAL ANALYSIS:")
    print(f"Total Lower Band Signals: {aggregate_metrics['total_lower_band_signals']}")
    print(f"Trades From Lower Band: {aggregate_metrics['total_trades_from_lower_band']}")
    ...
    print(f"Upper Band Exits: {aggregate_metrics['total_upper_band_exits']}")
...
if __name__ == '__main__':
    results_file, results = run_sprint_6_bollinger_test()
    print(f"\nSprint #6 Bollinger Band Mean Reversion test complete.")